...
2. **`package-editor`**: Custom Module untuk manajemen paket wisata bergaya **Visual Page Builder (ala Elementor / WordPress Gutenberg)**. Di halaman daftar, menggunakan layout tabel ala artikel dengan top search bar, tombol aksi ikon tunggal (`+`, `edit`, `delete`), dan filter status di sidebar kiri (#navigation). Di mode editor, menerapkan *inline editing* langsung pada Hero Banner, deskripsi WYSIWYG dengan TipTap, builder interaktif untuk Itinerary, Fasilitas, Tabel Harga Dinamis, serta Galeri Foto. Termasuk fitur **JSON Import** untuk *bulk import* atau *single override* paket wisata. Menerapkan Aturan Emas Z-Index (`z-index: 150`). Pada sisi frontend (`[slug].astro`), dilengkapi fitur **Dynamic PDF Itinerary Generator** menggunakan `pdfmake` murni (tanpa html2canvas) untuk mencetak penawaran yang 100% *vector* dan *robust*.
